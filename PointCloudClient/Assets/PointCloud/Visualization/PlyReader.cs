...
/// <summary>
/// Reads PLY-files and creates a Mesh from them
/// </summary>
public class PlyReader
{
    ...
    /// <summary>
    /// Class representing Header of the PLY-file stored as a List
    /// </summary>
    ...
    /// <summary>
    /// Class representing Body of the PLY-file. Stores the vertices (position) and colors of every point from one point cloud.
    /// </summary>
    ...
    /// <summary>
    /// Creates a mesh from a PLY-file from the file system
    /// </summary>
    /// <param name="path">Path to the PLY-file</param>
    /// <returns>
    /// A mesh of type <c>Mesh</c>
    /// </returns>
    public Mesh ImportAsMesh(string path)
    {
        var stream = File.Open(path, FileMode.Open, FileAccess.Read, FileShare.Read);
        var name = Path.GetFileNameWithoutExtension(path);
        return ImportAsMesh(stream, name);

    }

    /// <summary>
    /// Creates a mesh from a PLY-file from an URL
    /// </summary>
    /// <param name="url">URL to the PLY-file</param>
    /// <returns>
    /// A mesh of type <c>Mesh</c>
    /// </returns>
    public Mesh ImportAsMeshFromUrl(string url)
    {
        SimpleTimer.Begin("Download Mesh");

        WebClient webClient = new WebClient();
        var plyStream = webClient.OpenRead(url);

        var memoryStream = new MemoryStream();
        plyStream.CopyTo(memoryStream);
        memoryStream.Position = 0;

        SimpleTimer.End("Download Mesh");

        var name = Path.GetFileNameWithoutExtension(url);
        return ImportAsMesh(memoryStream, name);
    }

    /// <summary>
    /// Creates a mesh from a PLY-file read from a <c>Stream</c>
    /// </summary>
    /// <param name="stream">Steam to read the PLY-file from</param>
    /// <param name="name">Name used for the mesh</param>
    /// <returns>
    /// A mesh of type <c>Mesh</c>
    /// </returns>
    public Mesh ImportAsMesh(Stream stream, string name)
    {
        try
        {
            SimpleTimer.Begin("Read PLY Data");
            var header = ReadDataHeader(new StreamReader(stream));
            var body = ReadDataBody(header, new BinaryReader(stream));
            SimpleTimer.End("Read PLY Data");

            SimpleTimer.Begin("Create Mesh");
            var mesh = new Mesh();
            mesh.name = Path.GetFileNameWithoutExtension(name);
            ...
            SimpleTimer.End("Create Mesh");
            return mesh;
        }
        catch (Exception e)
        {
            Debug.LogError("Failed importing " + name + ". " + e.Message);
            return null;
        }
    }
